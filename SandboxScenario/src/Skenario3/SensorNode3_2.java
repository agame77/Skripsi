...
public class SensorNode3_2 {
	...
	// Setting Sensor 
	private static final String sensorId = "Sensor3";
	private static int SN_Addr = node_list[4];
	private static int BS_Addr = node_list[0];
	private static int[] nextNode = {  };
	private static int[] previousNode = { node_list[2] };
	
	//variable untuk sensing dan ekstraksi
	...
	public static void runs() {
		try {
			AT86RF231 t = Node.getInstance().getTransceiver();//pemancar
			t.open();
	...
	public static void receive(final FrameIO fio) {	
		Thread thread = new Thread() {
			public void run() {
				...
				Extract extract = new Extract(connection,(new String[]{sensorId}));
